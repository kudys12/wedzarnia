// web_server.cpp - [NEW] HTTP Basic Auth dla endpointów akcji
...
<a class="footer-link" href="/sd"><span class="fi">💾</span>Karta SD</a>
...
document.getElementById('reloadSdBtn').style.display = currentProfileSource === 'sd' ? 'inline-block':'none';
loadProfiles();
}
...
fetch('/profile/reload').then(r =>{if(!r.ok)alert('Błąd odczytu karty SD!');loadProfiles();fetchStatus();});
}
}
loadProfiles();
fetchStatus();
...
updateHeader();               // ustaw od razu
setInterval(updateHeader,1000); // aktualizacja co sekundę
...
<div class="btn-row">
<button class="btn-save" onclick="saveProfile()">💾 Zapisz na karcie SD</button>
...
// =================================================================
// KARTA SD – ujednolicony CSS
// =================================================================
static void handleSdPage() {
    if (!requireAuth()) return;
static const char HTML_SD[] PROGMEM = R"rawliteral(<!DOCTYPE html>
<html lang="pl">
<head>
<meta charset='utf-8'>
...
<div class="page-wrap">
<div class="page-header">
<h2>💾 Zarządzanie kartą SD</h2>
</div>
<div class="card" id="sdInfo">
<h3>Informacje o karcie</h3>
<div class="row"><span class="lbl">Status</span><span class="val" id="sdStatus">Sprawdzanie...</span></div>
<div class="row"><span class="lbl">Typ</span><span class="val" id="sdType">-</span></div>
<div class="row"><span class="lbl">Pojemność</span><span class="val" id="sdSize">-</span></div>
<div class="row"><span class="lbl">Użyte</span><span class="val" id="sdUsed">-</span></div>
<div class="row"><span class="lbl">Wolne</span><span class="val" id="sdFree">-</span></div>
</div>
<div class="warn-box card">
<h3>⚠️ Formatowanie karty SD</h3>
<p>Operacja <strong>nieodwracalnie usunie</strong>wszystkie dane na karcie.</p>
...
<input type="checkbox" id="chk" onchange="toggleBtn()">
<span>Rozumiem – chcę sformatować kartę SD(FAT32)</span>
</label>
<button id="btnFormat" disabled onclick="startFormat()">🗑️ Formatuj kartę SD</button>
<div id="progress">
<div id="bar"><div id="fill"></div></div>
<div id="msg">Formatowanie...</div>
</div>
<a class="back-link" href="/">⬅️ Wróć do strony głównej</a>
</div>
<script>
...
.then(r =>r.json())
.then(d =>{
document.getElementById('sdStatus').textContent = d.ok ? '✅ OK':'❌ Brak karty';
document.getElementById('sdType').textContent = d.type || '-';
document.getElementById('sdSize').textContent = d.size || '-';
document.getElementById('sdUsed').textContent = d.used || '-';
document.getElementById('sdFree').textContent = d.free || '-';
if(d.ok && d.idle){
...
.catch(()=>{document.getElementById('sdStatus').textContent = '❌ Błąd';});
function toggleBtn(){
...
if(!confirm('OSTATNIA SZANSA:Naprawdę sformatować kartę SD?\nWszystkie dane zostaną utracone!'))return;
...
.catch(()=>{clearInterval(timer);document.getElementById('msg').textContent = '❌ Błąd połączenia';});
    ...
    server.send_P(200, "text/html", HTML_SD);
}
...
// =================================================================
// CZUJNIKI – ujednolicony CSS (placeholder – uzupełnij logiką)
// =================================================================
...
<div class="row"><span class="lbl">Liczba czujników</span><span class="val" id="totalSensors">-</span></div>
<div class="row"><span class="lbl">Czujnik komory(idx)</span><span class="val" id="chamberIdx">-</span></div>
<div class="row"><span class="lbl">Czujnik mięsa(idx)</span><span class="val" id="meatIdx">-</span></div>
<div class="row"><span class="lbl">Zidentyfikowane</span><span class="val" id="identified">-</span></div>
</div>
<div class="card">
<h3>Przypisz ręcznie</h3>
<label>Indeks czujnika komory</label>
<input type="number" id="chamberInput" min="0" value="0">
<label>Indeks czujnika mięsa</label>
<input type="number" id="meatInput" min="0" value="1">
<div class="btn-row">
...
document.getElementById('chamberIdx').textContent = d.chamber_index;
document.getElementById('meatIdx').textContent = d.meat_index;
document.getElementById('identified').textContent = d.identified ? '✅ Tak':'❌ Nie';
});
}
function reassign(){
const c = document.getElementById('chamberInput').value;
const m = document.getElementById('meatInput').value;
const body = new URLSearchParams({chamber:c,meat:m});
...
// =================================================================
// INFORMACJE SYSTEMOWE /sysinfo – dane identyczne z ekranem TFT
// =================================================================
...
<h3>Peryferia</h3>
<div class="row">
<span class="lbl">💾 Karta SD</span>
<span class="val" id="sd_status">...</span>
</div>
<div class="row">
<span class="lbl">📏 Pojemność SD</span>
<span class="val" id="sd_size">...</span>
</div>
<div class="row">
...
const sdOk = d.sd_ok;
setVal('sd_status',sdOk ? '✅ OK('+d.sd_type+')':'❌ Brak karty',sdOk ? 'ok':'err');
setVal('sd_size',sdOk ? fmtBytes(d.sd_total)+' / wolne '+fmtBytes(d.sd_free):'-');
...
// Handler strony HTML – wymaga autoryzacji
static void handleSysInfoPage() {
    if (!requireAuth()) return;
    server.send_P(200, "text/html", HTML_SYSINFO);
}

// Handler JSON – dostarcza aktualne dane systemowe
static void handleSysInfoJson() {
    ...
    // Temperatura wewnętrzna ESP32 (dostępna przez esp_internal_temp)
    // Temperatura wewnętrzna ESP32 – dostępna w Arduino core >= 2.0
    // Wartość przybliżona (+/- kilka stopni), przydatna do monitoringu
    ...
    uint64_t sdFree  = sdOk ? (SD.totalBytes() - SD.usedBytes()) : 0;
    const char* sdType;
    switch (sdOk ? SD.cardType() : CARD_NONE) {
        case CARD_MMC:  sdType = "MMC";   break;
        case CARD_SD:   sdType = "SD";    break;
        case CARD_SDHC: sdType = "SDHC";  break;
        default:        sdType = "?";     break;
    }
    ...
    // MAC adres – przez Arduino WiFi API (działa na każdej wersji esp32 core)
    String macString = WiFi.macAddress();  // zwraca "XX:XX:XX:XX:XX:XX"
    const char* macStr = macString.c_str();

    // --- Skonstruuj JSON (static bufor – wystarczy ok. 600 B) ---
    ...
    snprintf(json, sizeof(json),
        ...
        sdOk ? "true" : "false", sdType, sdTotal, sdFree,
        sensorCount,
...
// =================================================================
// CZUJNIKI – handlery API (logika bez zmian)
// =================================================================
...
// =================================================================
// KARTA SD – handlery API (logika bez zmian)
// =================================================================
...
static void handleSdInfo() {
    ...
    if (!cardOk) {
        ...
        switch (SD.cardType()) {
            ...
            default:        typeStr = "Nieznany"; break;
        }
        auto fmtSize = [](uint64_t bytes, char* buf, size_t len) {
            if (bytes >= 1024*1024*1024ULL)
                snprintf(buf, len, "%.1f GB", bytes / (1024.0*1024.0*1024.0));
        ...
        snprintf(json, sizeof(json),
            "{\"ok\":true,\"idle\":%s,\"type\":\"%s\","
            "\"size\":\"%s\",\"used\":\"%s\",\"free\":\"%s\"}",
            isIdle ? "true" : "false",
            typeStr, sizeStr, usedStr, freeStr);
    }
    server.send(200, "application/json", json);
}
...
static void handleSdFormat() {
    ...
    if (SD.cardType() == CARD_NONE) {
        server.send(200, "application/json",
            "{\"ok\":false,\"message\":\"Brak karty SD!\"}");
        return;
    }
    LOG_FMT(LOG_LEVEL_WARN, "SD FORMAT requested via HTTP by authenticated user");
    SD.end();
    delay(500);
    ...
    f_unmount("");
    FRESULT fr = f_mkfs("", &opt, workBuf, sizeof(workBuf));
    if (fr == FR_OK) {
        delay(200);
        if (SD.begin(PIN_SD_CS)) {
            SD.mkdir("/profiles");
            SD.mkdir("/backup");
            LOG_FMT(LOG_LEVEL_INFO, "SD format OK, directories recreated");
            server.send(200, "application/json",
                "{\"ok\":true,\"message\":\"Karta sformatowana! Utworzono /profiles i /backup.\"}");
        } else {
            LOG_FMT(LOG_LEVEL_ERROR, "SD reinit failed after format");
            server.send(200, "application/json",
                "{\"ok\":false,\"message\":\"Sformatowano, ale reinicjalizacja nieudana – uruchom ponownie.\"}");
        }
    } else {
        SD.begin(PIN_SD_CS);
        LOG_FMT(LOG_LEVEL_ERROR, "SD format FAILED, FRESULT=%d", (int)fr);
        server.send(200, "application/json",
            "{\"ok\":false,\"message\":\"Formatowanie nieudane. Sprawdź kartę SD.\"}");
    }
}
...
void web_server_init() {
    ...
    server.on("/profile/reload", HTTP_GET, []() {
        ...
        if (storage_reinit_sd()) {
            ...
            server.send(500, "text/plain", "Błąd reinicjalizacji karty SD!");
        }
    });
    ...
    server.on("/wifi", HTTP_GET, []() {
        if (!requireAuth()) return;
        // Wstaw aktualne SSID do szablonu
    ...
    // =================================================================
    // OTA UPDATE – poprawiona obsługa dla FreeRTOS + esp32 Arduino core 3.x
    //
    // Problemy ze starym kodem:
    // 1. esp_task_wdt_deinit() nie usuwa taskWeb z WDT – taskWeb dalej
    //    woła esp_task_wdt_reset() co może crashować podczas uploadu.
    // 2. esp_task_wdt_init() po upload może nie działać (WDT już istnieje).
    // 3. Brak ochrony przed równoległym dostępem innych tasków podczas OTA.
    //
    // Rozwiązanie:
    // – Używamy esp_task_wdt_reset() w upload handlerze zamiast deinit.
    // – Przed restartem używamy esp_task_wdt_reconfigure() z długim timeout.
    // – Update.begin() z jawnym rozmiarem flash partycji.
    // =================================================================
    server.on("/update", HTTP_POST,
        // Handler odpowiedzi (po zakończeniu uploadu)
        []() {
            if (!requireAuth()) return;
        ...
        // Handler uploadu (wywoływany wielokrotnie podczas przyjmowania danych)
        []() {
            // Autoryzacja – sprawdzamy przy każdym wywołaniu
            if (!server.authenticate(storage_get_auth_user(), storage_get_auth_pass())) {
                server.requestAuthentication(BASIC_AUTH, "Wedzarnia", "Wymagane logowanie");
                return;
            }
            ...
            if (upload.status == UPLOAD_FILE_START) {
                ...
                // Wydłuż timeout WDT na czas całego uploadu (60s)
                // Używamy reconfigure zamiast deinit/init – bezpieczne gdy WDT już działa
                esp_task_wdt_config_t wdt_cfg = {
                    .timeout_ms     = 60000,   // 60 sekund na upload
                    .idle_core_mask = 0,        // nie monitoruj idle tasków
                    .trigger_panic  = false,    // nie crashuj – tylko log
                };
                esp_task_wdt_reconfigure(&wdt_cfg);

                // Zresetuj WDT dla tego taska żeby nie timeoutował podczas begin()
                esp_task_wdt_reset();

                // Zacznij aktualizację – UPDATE_SIZE_UNKNOWN = całe flash
                if (!Update.begin(UPDATE_SIZE_UNKNOWN)) {
                    Serial.printf("[OTA] begin() error: %s\n", Update.errorString());
                }

            } else if (upload.status == UPLOAD_FILE_WRITE) {
                // Resetuj WDT podczas każdego chunka – upload może trwać długo
                esp_task_wdt_reset();
                ...
                if (Update.end(true)) {   // true = ustaw boot partition
            ...
            // yield() pozwala taskowi webowemu obsłużyć inne callbacki
            yield();
        }
    );
